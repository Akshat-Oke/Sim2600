    ...
    for (const t2 of transdefs) {
      if (transistorsAreInSameBlock(t1, t2)) {
        for (const t3 of grounded) {
          if (transistorsAreInSameBlock(t2, t3)) {
...
function checkIfWiresFormGate({ gateType, in1, in2, out, elementToAdd }) {
  ...
  stream.on('data', function (line) {
    ...
    if (wires[out] != gateFunc[gateType](wires[in1], wires[in2])) {
      // console.log("Failed at line", lineCount, "for", in1, in2, wires[in1], wires[in2], "wire values, got: ", wires[out])
      valid = false;
      stream.end();
    }
  }).on('end', function () {
    ...
    if (valid) {
      ...
      fs.appendFileSync("nand_gates.txt", elementToAdd.transistorsInOrder[0][0] + ", " +
...
(async function () {
  ...
  for (const gate of nandGates) {
    ...
    // fs.appendFileSync('nand_gates.txt', `${gate.transistorsInOrder[0][0]}, ${gate.transistorsInOrder[1][0]}, ${gate.transistorsInOrder[2][0]}`
    // );
    checkIfWiresFormGate({ gateType: "NAND", in1, in2, out, elementToAdd: gate });
  }
